 ...
 * @copyright 2017-19 (c) Thomas Malt
 * @license MIT
 */

const { api, hosts } = require('../config')
...
// w.cli();
w.info(`Starting cloudflare DDNS updater v${version}`)
  ...
  for (let item of records.data.result) {
    ...
    if (hosts[item.name].types.includes(item.type)) {
      if (item.content === records.addr[item.type]) {
        ...
        results.updates[results.updates.length] = cf
          .updateDNSRecord({
            addr: records.addr,
            type: item.type,
...
cf.fetchIPv4Addr = () => {
  ...
  return request('https://ipinfo.io/ip')
    ...
    .then(addr => {
      ...
      w.info(` Extra - ${net.ipv4({ iface: 'eth0' })} `)
